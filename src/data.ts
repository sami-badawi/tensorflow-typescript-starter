...
export async function fetchOnceAndSaveToDiskWithBuffer(filename: string): Promise<Buffer> {
...
export function loadHeaderValues(buffer, headerLength: number) {
  const headerValues = [];
...
export async function loadImages(filename: string) {
...
export async function loadLabels(filename: string) {
...
export class MnistDataset {
  ...
  public getData_(isTrainingData: boolean) {
    let imagesIndex;
    let labelsIndex;
    if (isTrainingData) {
      imagesIndex = 0;
